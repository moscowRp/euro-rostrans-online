const API = "https://euro-api.onrender.com";
...
async function doRegister(){
  ...
  const logist_code = $("regLogistCode") ? $("regLogistCode").value.trim() : "";

  try{
    const r = await api("/api/auth/register", {
      method:"POST",
      body: JSON.stringify({ username, password, role, logist_code })
    });
    setToken(r.token);
    setUser(r.user);
...
async function init(){
  ...
  // показываем поле кода только если выбрали LOGIST
  $("regRole").onchange = () => {
    const isLogist = $("regRole").value === "LOGIST";
    const el = $("regLogistCode");
    if (!el) return;
    el.classList.toggle("hidden", !isLogist);
  };
